...
def showProbeSpectre(probes: List[Probe],
                     dx: float, dt: float, minYSize: float, maxYSize: float):
    '''
    Показать графики сигналов, зарегистрированых в датчиках.
    ...
    for n, (probe, ax) in enumerate(zip(probes, axes_list)):
        ...
        # Настройка внешнего вида графиков
        ax.set_xlim(0, 6e9)
        ax.set_ylim(np.min(fft), np.max(fft) + 1)
        ax.set_xlabel('f, Hz')
...
def showGamma(probes: List[Probe],
                     dx: float, dt: float, minYSize: float, maxYSize: float):
    ...
    fft_falling = np.abs(np.fft.fft(probes[1].E))
    fft_reflected = np.abs(np.fft.fft(probes[0].E))

    fftfreq = np.fft.fftfreq(len(probes[0].E), dt)
