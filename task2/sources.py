...
class SourceGauss(metaclass=ABCMeta):
    @abstractmethod
    def getE(self, position, time):
        pass
...
class SourceTFSFG(Source1D):
    def __init__(self, source: SourceGauss,
                 sourcePos: float,
                 Sc: float = 1.0,
                 eps: float = 1.0,
                 mu: float = 1.0):
    ...
    def getH(self, time):
        return -(self.Sc / (self.W0 * self.mu)) * self.source.getE(time)
    
    def getE(self, time):
        return (self.Sc / np.sqrt(self.eps * self.mu)) * self.source.getE(time + 0.5)
    

class Gaussian(SourceGauss):
    '''
    Источник, создающий гауссов импульс
    '''

    def __init__(self, magnitude, dg, wg, Sc=1.0, eps=1.0, mu=1.0):
        '''
    ...
    def make_continuous(magnitude: float,
                        ...
                        Sc: float,
                        eps: float = 1.0,
                        mu: float = 1.0) -> 'Gaussian':
        # Количество ячеек на длину волны
        Nl = Sc / (freq * dt)
        phi_0 = -2 * np.pi / Nl
        return Gaussian(magnitude, Nl, phi_0, Sc)


class GaussianDiff(SourceGauss):
    '''
